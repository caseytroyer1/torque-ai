...
def analyze_frames_with_claude(address_frame_b64, backswing_frame_b64, impact_frame_b64, mediapipe_data=None, golfer_hand='right', golfer_club='iron', user_camera_angle='down_the_line'):
    ...
    if knee_flex is not None:
        mp_facts.append(f"- Knee flex: MediaPipe measured {knee_flex}° but this measurement is unreliable from DTL — IGNORE this number completely. Judge knee flex purely by looking at the address frame image. A good golf setup has a slight, athletic knee bend — the golfer looks ready to move, not squatting and not locked straight. Most recreational golfers with decent form have appropriate knee flex. Only return 'too bent' if the golfer is clearly squatting deeply, or 'too straight' if legs look completely locked. Return your verdict in knee_flex_visual as exactly one of: good / slightly bent / too bent / too straight")
    if shoulder_level is not None:
  ...
  "impact": {
    ...
    "hip_slide": "good/too much/insufficient",
    "finish_position": "balanced/falling back/falling forward",
  ...
  "overall": {
    ...
    instruction_text = (
        ...
        f"LATERAL HEAD MOVEMENT CRITERIA — use these for the lateral_head_movement field (face-on, backswing frame):\n"
        f"- 'steady': head stays centered over the ball throughout the backswing. Ideal.\n"
        f"- 'swayed left': head has moved toward the target during the backswing. Causes reverse pivot.\n"
        f"- 'swayed right': head has moved away from target during backswing. Common — slight movement is acceptable, only flag obvious sway.\n"
        f"Important: slight head movement is normal. Only flag clear and obvious lateral movement.\n"
        f"WEIGHT TRANSFER BACKSWING CRITERIA — use these for the backswing weight_transfer field (face-on):\n"
        f"- 'good': weight appears to load onto the trail side during the backswing, golfer looks coiled and ready.\n"
        f"- 'minimal': weight stays centered, not loading trail side — reduces power potential.\n"
        f"- 'reverse pivot': weight moving toward lead side during backswing — major fault, will cause topped and thin shots.\n"
        f"Important: most recreational golfers load weight reasonably well. Only flag 'reverse pivot' for obvious cases.\n"
        f"SHOULDER TILT AT IMPACT CRITERIA — use these for the shoulder_tilt field (face-on, impact frame):\n"
        f"- 'good': lead shoulder is higher than trail shoulder at impact, classic tilted position showing proper rotation.\n"
        f"- 'too flat': shoulders appear level at impact, insufficient tilt, indicates blocked rotation.\n"
        f"- 'too steep': lead shoulder dramatically higher than trail, over-rotation or excessive tilt.\n"
        f"Important: some shoulder tilt at impact is always correct. Only flag 'too flat' when shoulders look nearly level.\n"
        f"HEAD POSITION AT IMPACT CRITERIA — use these for the head_position field (face-on, impact frame):\n"
        f"- 'steady': head has stayed roughly centered over the ball through impact. Ideal.\n"
        f"- 'moved forward': head has moved significantly toward the target by impact.\n"
        f"- 'moved back': head has moved away from target — may indicate hanging back.\n"
        f"Important: slight forward movement is acceptable and natural. Only flag obvious movement.\n"
        f"WEIGHT TRANSFER IMPACT CRITERIA — use these for the impact weight_transfer field (face-on):\n"
        f"- 'good': weight has fully shifted to the lead side at impact, golfer looks like weight is on lead foot.\n"
        f"- 'minimal': weight still centered or split at impact, not fully transferred.\n"
        f"- 'reversed': weight still on trail side at impact — major fault causing all kinds of miss-hits.\n"
        f"HIP SLIDE CRITERIA — use these for the hip_slide field (face-on, impact frame):\n"
        f"- 'good': hips have shifted laterally toward the target while also rotating, creating a powerful bump and turn.\n"
        f"- 'too much': hips have slid excessively toward target with little rotation, causes blocks and pushes.\n"
        f"- 'insufficient': hips have barely moved toward target, rotation only, causes pulls and over-the-top moves.\n"
        f"FINISH POSITION CRITERIA — use these for the finish_position field (face-on, impact frame):\n"
        f"- 'balanced': golfer looks balanced on the lead foot at finish, weight fully transferred, classic finish.\n"
        f"- 'falling back': golfer falling toward trail side through impact and finish, classic hanging back fault.\n"
        f"- 'falling forward': golfer falling toward target excessively, losing posture forward.\n"
        f"Important: most golfers with decent mechanics finish balanced. Only flag obvious balance issues.\n\n"
